...
def predict_price(s_date, e_date, ticker: str):
    ...
    # if len(all_data) == 0 or np.isnan(all_data[10][0]):
    #    return
...
# csv_data = pd.read_csv("just_tickers.csv", header=0)
...
data = ['AAPL', 'MTRN', 'QLCM', 'TSLA', 'NVDA', 'F', 'GE']


for k in data:
    try:
        predict_price("2012-1-1", "2020-10-25", k)
    except IndexError:
        pass
